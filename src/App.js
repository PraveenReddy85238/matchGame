...
const App = () => {
  ...
  const filterListBasedOnTab = id => {
    setActiveTab(id)
  }

  const filterList = imagesList.filter(each => each.category === activeTab)
  const randomImage = imagesList[randomNumber].imageUrl

  const activeMatchId = id => {
    const selectedId = imagesList.find(each => each.id === id)
    const random = Math.ceil(Math.random() * imagesList.length)
    if (randomImage === selectedId.imageUrl) {
  ...
  return (
    <>
      ...
      <div className="bg-container">
        <img src={randomImage} className="random-image" alt="match" />
        <ul className="tabs-list-items-container">
          {tabsList.map(eachTab => (
            <TabsList
              eachTab={eachTab}
              key={eachTab.id}
              filterListBasedOnTab={filterListBasedOnTab}
            />
          ))}
        </ul>
